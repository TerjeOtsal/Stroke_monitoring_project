...
# Plot de første 20 filtrerte punktene i 3D uten linjer
x = filtered_df['filtered_x'].iloc[1:21]  # De første 20 punktene (unntatt startposisjon)
y = filtered_df['filtered_y'].iloc[1:21]
z = filtered_df['filtered_z'].iloc[1:21]
ax.scatter(x, y, z, color='blue', s=5, label='Filtrert armposisjon (første 20 punkter)')  # 's' er størrelsen på punktene

# Etiketter
ax.set_xlabel('X posisjon (meter)')
ax.set_ylabel('Y posisjon (meter)')
ax.set_zlabel('Z posisjon (meter)')
ax.set_title('Armens bevegelse i 3D over tid (første 20 punkter)')
ax.legend()
