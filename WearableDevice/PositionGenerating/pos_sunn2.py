...
# Konverter posisjonene til numpy array
positions = np.array(positions)

# Sett startposisjonen til (0, 0, 0) og skaler de andre i forhold til startposisjonen
start_position = positions[0]
relative_positions = positions - start_position  # Skaler alle posisjoner i forhold til startposisjon

# Steg 3: Kalman-filter for glatting (bruker relative posisjoner)
...
# Starttilstander
initial_state_mean = np.hstack([relative_positions[0], np.zeros(3)])  # Start i (0, 0, 0) med null-hastighet
...
# Kjør Kalman-filter på relative posisjonsdata
filtered_state_means, filtered_state_covariances = kf.filter(relative_positions)
...
ax.scatter(0, 0, 0, color='red', s=50, label='Startposisjon')  # Startpunkt er nå (0, 0, 0)
