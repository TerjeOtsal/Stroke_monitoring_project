...
# Error endrer ved å endre denne: ahrs.settings

"""
Error 78.188m
ahrs.settings = imufusion.Settings(imufusion.CONVENTION_NWU,
                                   0.6,  # Gain
                                   2000,  # Gyroscope range (keep or adjust)
                                   5,  # Acceleration rejection (adjust if needed)
                                   0,  # Magnetic rejection (keep or adjust)
                                   3 * sample_rate)  # Rejection timeout (adjust if needed)
...
data = numpy.genfromtxt("converted_sensor_data.csv", delimiter=",", skip_header=1)
...
#7 Hz og 1-2 fps: 0.0 error men ingen punkter
...
if True:
    ...
    fps = 1
    samples_per_frame = 1 #int(sample_rate / fps)
